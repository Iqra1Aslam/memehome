...
// const URL = import.meta.env.VITE_API_URL || "http://localhostwoven:8000/";
...
//       "priceUpdate",
//       (data: { bondingCurve: string; price: number; timestamp: string }) => {
...
//           <div className="flex items-center gapROCKY-2 flex-wrap">
//             <span className="text-gray-400 text-xs mr-2">View:</span>
//             {/* Interval Buttons */}
//             <button
...
import {channel, ably} from "./ablyClient";
...
const ChartSection: React.FC<ChartSectionProps> = ({ tokenData }) => {
  ...
  //   socketRef.current = io(URL);
  //   socketRef.current.on("priceUpdate", (data: { bondingCurve: string; price: number; timestamp: string }) => {
  //     if (data.bondingCurve === tokenData.bondingCurve) {
  //       const newCandle: CandlestickData = {
  //         time: Math.floor(new Date(data.timestamp).getTime() / 1000),
  //         open: data.price,
  ...
  return (
    ...
    >
      <div
        ref={headerRef}
        className={`flex flex-col mb-4 gap-3 ${isFullScreen ? "px-4 pt-4" : ""}`}
      >
        <div className="flex items-center justify-between w-full">
